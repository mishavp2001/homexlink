...
import { getOAuthRedirectUri, isCapacitor } from '@/components/lib/platform-utils';
...
export const AuthProvider = ({ children }) => {
  ...
  const checkUserAuth = async ({ suppressAuthError = false } = {}) => {
    try {
      ...
      // Handle mobile deep link redirect after successful auth
      const urlParams = new URLSearchParams(window.location.search);
      const fromUrl = urlParams.get('from_url');
      
      if (fromUrl && fromUrl.startsWith('homexlink://')) {
  ...
  const navigateToLogin = async () => {
    const redirectUri = getOAuthRedirectUri(window.location.href);
    await base44.auth.redirectToLogin(redirectUri);
  };
