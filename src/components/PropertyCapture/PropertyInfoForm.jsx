...
export default function PropertyInfoForm({ initialData, onNext }) {
  ...
  const validateAndEnrichAddress = async (address) => {
    ...
    try {
      ...
      // Step 2: Try to fetch from Databright API first
      setLoading(true);
      ...
      try {
        ...
        if (databrightResponse.data.success && databrightResponse.data.property) {
          const prop = databrightResponse.data.property;
          enrichResult = {
            sqft: prop.sqft,
            ...
            rebuild_cost_per_sqft: null,
            land_value: null,
            appraised_value: prop.appraised_value,
            data_confidence: 'High (Databright)',
            data_sources: 'Databright Public Records',
            property_photos: prop.photos || []
          };
        }
      } catch (error) {
        console.log('Databright fetch failed, falling back to AI enrichment');
      }
...
Return realistic, data-backed values. If certain data is not available, provide reasonable estimates based on the neighborhood and property type.`,
      ...
      if (enrichResult) {
        ...
        setFormData(prev => ({
          ...
          property_photos: enrichResult.property_photos || prev.property_photos,
          appraised_value: enrichResult.appraised_value || prev.appraised_value,
        }));
      }
    } catch (error) {
  ...
  return (
    <Card className="p-8 max-w-3xl mx-auto bg-white shadow-xl">
        ...
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              {formData.appraised_value && (
                <p className="text-sm text-green-800 font-bold mt-2">
                  Zestimate: ${Number(formData.appraised_value).toLocaleString()}
                </p>
              )}
      ...
      {formData.property_photos && formData.property_photos.length > 0 && (
        <div className="mb-6">
          <Label className="mb-3 block">Property Photos from Zillow</Label>
          <div className="grid grid-cols-3 gap-3">
            {formData.property_photos.map((photo, idx) => (
              <div key={idx} className="relative aspect-video rounded-lg overflow-hidden border-2 border-blue-200">
                <img
                  src={photo}
                ...
                />
                {idx === 0 && (
                  <Badge className="absolute top-2 left-2 bg-blue-600 text-white">Main Photo</Badge>
                )}
              </div>
            ))}
          </div>
