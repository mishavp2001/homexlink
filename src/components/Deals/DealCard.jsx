...
export default function DealCard({ deal, onViewDetails, onMakeOffer, isOwner }) {
  ...
  // Load deal owner info
  React.useEffect(() => {
    const loadOwner = async () => {
      if (!deal.user_email || (isProperty)) {
        return;
      }
      try {
        ...
        if (error.message?.includes('not found') || error.response?.status === 500) {
          setDealOwner(null);
        } else {
  ...
  const saveDealMutation = useMutation({
    ...
    onMutate: async (newSave) => {
      ...
      const previousSaves = queryClient.getQueryData(['savedDeals', currentUser?.email, deal.id]);
      
      // Optimistically update
      queryClient.setQueryData(['savedDeals', currentUser?.email, deal.id], [newSave]);
      
      return { previousSaves };
    },
    onError: (err, newSave, context) => {
      // Rollback on error
      queryClient.setQueryData(['savedDeals', currentUser?.email, deal.id], context.previousSaves);
    },
    onSettled: () => {
  ...
  const unsaveDealMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      // Optimistically update
      queryClient.setQueryData(['savedDeals', currentUser?.email, deal.id], []);
  ...
  const isProperty = ['property_sales', 'long_term_rent', 'short_term_rent'].includes(deal.deal_type);
  const isSale = deal.deal_type === 'property_sales';
  ...
  const getPublicDealUrl = () => {
    ...
    if (deal.deal_type === 'property_sales') {
  ...
  const getDealTypeLabel = () => {
    switch (deal.deal_type) {
      ...
      case 'short_term_rent':
        return 'Airbnb';
      case 'service_deal':
        if (deal.is_combo_deal && deal.service_types?.length > 1) {
          return '🎁 Combo Deal';
        }
        return deal.service_types?.length === 1 ? deal.service_types[0] : (deal.service_category || 'Service Deal');
      default:
  ...
  const getDiscountPercent = () => {
    ...
    if (deal.combo_discount_percent) return deal.combo_discount_percent;
    if (deal.original_price && deal.price && deal.original_price > deal.price) {
    ...
    <Card className="overflow-hidden hover:shadow-xl transition-shadow">
      {deal.photo_urls?.length > 0 && (
        ...
        >
          ...
          {discountPercent && (
            <div className="absolute bottom-3 left-3 bg-red-500 text-white px-3 py-2 rounded-lg text-sm font-bold shadow-lg">
              {discountPercent}% OFF
            </div>
          )}
      ...
      <div className="p-6">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-1 cursor-pointer hover:text-[#2a4a7f] leading-tight" onClick={() => onViewDetails(deal)}>
          {deal.title}
        </h3>

        {dealOwner?.business_name && (
          <Link 
            to={createPageUrl(`PublicProfile?business=${encodeURIComponent(dealOwner.business_name)}`)}
        ...
        <div className="flex items-center gap-2 mb-3">
          {discountPercent ? (
            <>
              <span className="text-lg text-gray-400 line-through">${deal.original_price?.toLocaleString()}</span>
              <span className="text-xl font-bold text-gray-900">${deal.price?.toLocaleString()}</span>
        ...
        {!isProperty && (
          <div className="mb-3 flex flex-wrap gap-2">
            {deal.is_combo_deal && deal.service_types?.length > 1 ? (
              <>
            ...
            ) : deal.service_types?.length > 0 ? (
              <Badge variant="outline">
                {deal.service_types[0]}
              </Badge>
        ...
        <p className="text-gray-600 text-sm mb-4 line-clamp-2">
          {deal.description ? deal.description.replace(/<[^>]+>/g, '') : ''}
        </p>
