...
// PROJECT_TYPES will be loaded from categories
...
export default function Maintenance() {
  ...
  const createProjectMutation = useMutation({
    ...
    onMutate: async (newProject) => {
      ...
      await queryClient.cancelQueries(['projects', propertyId]);
      
      // Snapshot previous value
      const previousProjects = queryClient.getQueryData(['projects', propertyId]);
      ...
      queryClient.setQueryData(['projects', propertyId], [optimisticProject, ...(previousProjects || [])]);
      
      return { previousProjects };
    },
    onError: (err, newProject, context) => {
      // Rollback on error
      queryClient.setQueryData(['projects', propertyId], context.previousProjects);
    },
    ...
    onSettled: () => {
      queryClient.invalidateQueries(['projects', propertyId]);
    }
  });
...
HomeXrei Team`,
  ...
  const handleRefresh = async () => {
    await queryClient.invalidateQueries(['projects', propertyId]);
  };
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <MobileSelect
                      ...
                      options={projectTypeCategories.length > 0 ? 
                        projectTypeCategories.map(type => ({
                          value: type.name,
                  ...
                  <div>
                    ...
                    <MobileSelect
                      ...
                      options={COMPONENT_TYPES.map(comp => ({
                        value: comp.value,
                        label: comp.label
                      }))}
                ...
                {components.length > 0 && (
                  <div>
                    ...
                    <MobileSelect
                      ...
                      options={[
                        { value: null, label: 'None - New component' },
                        ...components.map(comp => ({
                          value: comp.id,
                          label: `${comp.component_type} - ${comp.current_condition}${comp.serial_number ? ` (${comp.serial_number})` : ''}`
                        }))
                      ]}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <Badge variant="outline" className="capitalize">
                          {projectTypeCategories.find(t => t.name === project.project_type)?.icon} {project.project_type}
                        </Badge>
