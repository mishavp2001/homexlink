...
export default function Accounting() {
  ...
  const createTransactionMutation = useMutation({
    ...
    onMutate: async (newTransaction) => {
      ...
      await queryClient.cancelQueries(['transactions', propertyId]);
      
      // Snapshot previous value
      const previousTransactions = queryClient.getQueryData(['transactions', propertyId]);
      
      // Optimistically update with temporary transaction
      ...
      queryClient.setQueryData(['transactions', propertyId], [optimisticTransaction, ...(previousTransactions || [])]);
      
      return { previousTransactions };
    },
    onError: (err, newTransaction, context) => {
      // Rollback on error
      queryClient.setQueryData(['transactions', propertyId], context.previousTransactions);
    },
    onSuccess: () => {
      setShowForm(false);
      setFormData({
    ...
    onSettled: () => {
      queryClient.invalidateQueries(['transactions', propertyId]);
    }
  });
  ...
  const handleRefresh = async () => {
    await queryClient.invalidateQueries(['transactions', propertyId]);
  };
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <MobileSelect
                  ...
                  options={(formData.transaction_type === 'income' ? INCOME_CATEGORIES : EXPENSE_CATEGORIES).map(cat => ({
                    value: cat,
                    label: cat
                  }))}
